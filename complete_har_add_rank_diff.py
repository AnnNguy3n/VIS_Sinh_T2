                ...
                if idx + 1 == formula.shape[0]:
                    ...
                    for w_i in range(temp_0_new.shape[0]):
                        ...
                        for c_i in range(0, self.INDEX.shape[0]-1):
                            sub_value = weight[self.INDEX[-c_i-2]:self.INDEX[-c_i-1]]
                            sub_profit = self.PROFIT[self.INDEX[-c_i-2]:self.INDEX[-c_i-1]]
                            rank_value = np.argsort(np.argsort(sub_value))
                            rank_profit = np.argsort(np.argsort(sub_profit))
                            ...
                            if c_i >= self.test_start:
                                ...
                                har, har_L, value_har_L = nopy.har_har_L_value_har_L(value, index, profit, target)
                                if har >= target and har_L - har >= self.alpha:
                                    geo, geo_L, value_geo_L = nopy.geo_geo_L_value_geo_L(value, index, profit, 0.0)
